...
public class CommandNMTaxi extends Thread implements GlobalConstant,DriveMode{
    /*
    @Overview:出租车控制线程,通过出租车的当前状态位置以及是否被分配请求等,执行相关的调度处理以及状态
    位置转换,主要通过sleep()睡眠模拟出租车行驶
     */
    NMTaxi monitorNMTaxi;//监控的出租车
    private int taxiCode;//出租车编号
    PassengerRequest allocRequest;//被分配的请求
    double waitCount = 0.0;//等待服务时间计数器
    boolean checkRoadChange;//改路信号
    int frontPosition;//出租车之前的位置
    CommandNMTaxi(NMTaxi monitorNMTaxi, int taxiCode){
        /*@REQUIRES:monitorNMTaxi!=null && taxiCode>=0 && taxiCode<=99 && monitorNMTaxi.taxiCode==taxiCode
        @MODIFIES:this.monitorNMTaxi,this.taxiCode,this.allocRequest,this.checkRoadChange,this.frontPosition
        @EFFECTS:类对象构造
        */
        this.monitorNMTaxi = monitorNMTaxi;
    ...
    /*@repOk
    check:0<=taxiCode<=99; monitorNMTaxi!=null &&  monitorNMTaxi is an instance of NMTaxi && monitorNMTaxi.taxiCode == taxiCode;
    -1<=frontPosition<=6399 (由于一开始初始化在-1) && waitCount>=0.0 && (if monitorNMTaxi.status==GRAB_SERVICE~IN_SERVICE==>
    ...
    public boolean repOk(){
        ...
        if(monitorNMTaxi ==null) return false;
        if(waitCount<0.0) return false;
        Object x = monitorNMTaxi;
        if(!(x instanceof NMTaxi)) return false;
        if(monitorNMTaxi.getTaxiCode()!=taxiCode) return false;
        if(frontPosition<-1 || frontPosition>6399) return false;
        if(monitorNMTaxi.getCurrentStatus()==GRAB_SERVICE || monitorNMTaxi.getCurrentStatus()==STOP_GRAB ||
                monitorNMTaxi.getCurrentStatus()==IN_SERVICE){
    ...
    public void run(){
        ...
        */
        try {
            while (true) {
                int status = monitorNMTaxi.getCurrentStatus();
                //新增的两个停止状态将在
                switch (status) {
                    case WAIT_SERVICE://只有等待服务状态可以抢单
                        monitorNMTaxi.searchAblePick();
                        carryWaitService();
                        break;
                    case STOP_SERVICE:
                        carryStopService();
    ...
    void setAllocRequest(PassengerRequest get){
        ...
        @MODIFIES:this.allocRequest,this.monitorNMTaxi
        ...
        */
        ...
        monitorNMTaxi.setCurrentStatus(GRAB_SERVICE);
        monitorNMTaxi.clearHashSet();//清空哈希表
    }
    ...
    void setCheckRoadChange(){
        ...
        */
        if(monitorNMTaxi.getCurrentStatus()==GRAB_SERVICE || monitorNMTaxi.getCurrentStatus()==IN_SERVICE){
    ...
    private void carryWaitService() throws InterruptedException{
        ...
        @MODIFIES:this.waitCount,this.monitorNMTaxi,this.frontPosition
        ...
        */
        ...
        int nextPosition = randomNextPosition(monitorNMTaxi.getCurrentRow(), monitorNMTaxi.getCurrentCol(),
                monitorNMTaxi.getCurrentPosition());
        //排除第一次选择的流量减问题
        if(frontPosition>=0){
        ...
        Main.carFlow.addCarFlowAt(monitorNMTaxi.getCurrentRow(), monitorNMTaxi.getCurrentCol(),Main.getRowByCode(nextPosition),
                Main.getColByCode(nextPosition));
        sleep((long)(gridConsume*1000));
        //设置新坐标
        frontPosition = monitorNMTaxi.getCurrentPosition();
        monitorNMTaxi.setCurrentPosition(nextPosition);
        int status;
        ...
        if(monitorNMTaxi.getCurrentStatus()==WAIT_SERVICE)
            monitorNMTaxi.setCurrentStatus(status);
    }
    //停止状态
    private void carryStopService() throws InterruptedException{
        /*@REQUIRES:monitorNMTaxi!=null && monitorNMTaxi.taxiCode == this.taxiCode && Main.carFlow有效
        ...
        */
        ...
        monitorNMTaxi.setCurrentStatus(WAIT_SERVICE);
    }
    ...
    protected void carryGrabService() throws InterruptedException{
        ...
        */
        ...
        if(src == monitorNMTaxi.getCurrentPosition()){
            ...
            info+="("+ monitorNMTaxi.getCurrentRow()+","+ monitorNMTaxi.getCurrentCol()+")";
            List<Integer> shortestPath = Main.getShortestPath(monitorNMTaxi.getCurrentPosition(), src,false);
    ...
    protected void carryInService() throws InterruptedException{
        ...
        */
        ...
        List<Integer> shortestPath = Main.getShortestPath(src,dst,false);
        info+=driveByShortestPath(shortestPath);
        info+="\t OK,完成服务,已到达目的地.";
        ...
        */
        ...
        for(int i=shortestPath.size()-2;i>0;i--){
            ...
            if(checkRoadChange){
                ...
                shortestPath = Main.getShortestPath(monitorNMTaxi.getCurrentPosition(),shortestPath.get(1),false);
    ...
    void waitRedLight(int nextPosition) throws InterruptedException{
         ...
         */
        int currentPosition = monitorNMTaxi.getCurrentPosition();
        int mode = 0;//行驶模式识别
