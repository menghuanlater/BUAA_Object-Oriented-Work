...
public class Main implements GlobalConstant{
    ...
    public static void main(String[] args) {
        /*@REQUIRES:None
        @MODIFIES:matrix,matrixGui,gui,taxiSets,commandTaxis,safeFilePassenger,safeFileRequest.
        @EFFECTS:normal_behavior:初始化各相关的变量,读取加载地图,启动gui、出租车、请求输入、请求抓取处理等线程
    ...
    private static int getEdgeByPoint(int row1, int col1, int row2, int col2){//根据节点坐标找到边的编号
        /*@REQUIRES:0<=row1,col1,row2,col2<=79 &&((row1==row2 && Math.abs(col1-col2)==1)
                                || (col1==col2 && Math.abs(row1-row2)==1))
        @MODIFIES:None
        @EFFECTS:返回(row1,col1)<->(row2,col2)所代表的边的编号
        */
        if(row1==row2){
    ...
    static int getEdgeByPoint(int code1,int code2){//根据节点编码找到边的编码
        /*@REQUIRES:0<=code1,code2<=6399且code1与code2所代表的节点间距为1单位
        @MODIFIES:None
        @EFFECTS:返回code1<->code2所代表的边的编号
        */
    ...
    static void illegalMap(){
        /*@REQUIRES:Node
        @MODIFIES:System.out,System.exit()
    ...
    static synchronized List<Integer> getShortestPath(int startCode, int endCode, HashSet<Integer> passEdgeSets){
        /*@REQUIRES:0<=startCode,endCode<=6399 && passEdgeSets!==null;
        @MODIFIES:passEdgeSets,find[],pathArc[],shortPathTable[],costTable[]
        ...
        */
        ...
        for(int v=0;v<NODE_NUM;v++){
            ...
            costTable[v] = (matrix[startCode][v])? guigv.GetFlow(getRowByCode(startCode),getColByCode(startCode),
        ...
        for(int v=0;v<NODE_NUM;v++){
            ...
            for (Integer aRelateList : relateList) {
                int cost = costTable[k] + guigv.GetFlow(getRowByCode(k),
    ...
    static synchronized int getNearestTaxi(int srcPos, HashMap<Integer,Integer> availableTaxis){
        /*@REQUIRES:0<=srcPos<=6399 && availableTaxis,size()>=2// <=1没有任何需要判断的必要
        @MODIFIES:find[]
    ...
    static synchronized void modifyRoad(List<RoadRequest> list){
        ...
        @MODIFIES:closeRoadSets,matrix[][],gui,commandTaxis[]
        @EFFECTS:根据道路修改指令,关闭或打开某些路,关闭的路编号记录在closeRoadTaxis中,如果存在道路关闭,
                则立刻通知所有出租车检查是否需要改路径
        ...
        */
        ...
        for (RoadRequest aList : list) {
            ...
            if(!status) closeRoadSets.add(getEdgeByPoint(firstRow,firstCol,nextRow,nextCol));
            //设置gui状态
